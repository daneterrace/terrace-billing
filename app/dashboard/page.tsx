...
import { Wrench, Wifi, Zap, Calendar, AlertCircle, Clock, CheckCircle, TrendingUp, Building2, Users } from 'lucide-react'
import Link from 'next/link'

type MaintenanceJob = { id: string; title: string; status: string; created_at: string; centre: { name: string } | null }
type Tenant = { id: string; company_name: string; lease_end: string | null; centre: { name: string } | null }
type InternetStat = { centre_name: string; revenue: number; cost: number; tenant_count: number }
type GeneratorStat = { centre_name: string; total_kwh: number; total_cost: number }
...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    const [centresRes, tenantsRes, jobsRes, internetRes, genRes] = await Promise.all([
      supabase.from('centres').select('id, name').eq('is_active', true).order('name'),
      supabase.from('tenants').select('id, company_name, lease_end, centre:centres(name)').eq('status', 'active'),
      supabase.from('maintenance_jobs')
        .select('id, title, status, created_at, centre:centres(name)')
        .not('status', 'in', '("completed","rejected")')
        .order('created_at', { ascending: false })
        .limit(6),
      supabase.from('billing_line_items')
        .select('tenant_id, sell_price, cost_price, tenants(centre_id, centres(name))')
        .eq('utility_type', 'internet'),
      supabase.from('generator_monthly')
        .select('centre_id, total_kwh_generated, fuel_cost, opex_cost, centres(name)')
    ...
    // Expiring leases in next 60 days
    const in60 = new Date()
    in60.setDate(in60.getDate() + 60)
    const expiring = allTenants.filter((t: any) => {
    ...
    setExpiringTenants(expiring as any)

    // Internet stats by centre
    const items = internetRes.data ?? []
    const centreMap = new Map<string, InternetStat>()
    items.forEach((item: any) => {
      const centreName = item.tenants?.centres?.name
      if (!centreName) return
      const existing = centreMap.get(centreName) ?? { centre_name: centreName, revenue: 0, cost: 0, tenant_count: 0 }
      existing.revenue += item.sell_price ?? 0
      existing.cost += item.cost_price ?? 0
      existing.tenant_count++
      centreMap.set(centreName, existing)
    })
    setInternetStats(Array.from(centreMap.values()))

    // Generator stats
    const genData = (genRes.data ?? []).map((g: any) => ({
      centre_name: g.centres?.name ?? 'Unknown',
      total_kwh: g.total_kwh_generated ?? 0,
      total_cost: (g.fuel_cost ?? 0) + (g.opex_cost ?? 0),
    }))
    setGeneratorStats(genData)

    setLoading(false)
  }

  const totalRevenue = internetStats.reduce((sum, s) => sum + s.revenue, 0)
  const totalCost = internetStats.reduce((sum, s) => sum + s.cost, 0)
  const totalProfit = totalRevenue - totalCost
  const totalKwh = generatorStats.reduce((sum, s) => sum + s.total_kwh, 0)
  const activeTenants = tenants.filter((t: any) => t.status !== 'inactive')
    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm mt-1" style={{ color: '#888' }}>
          {now.toLocaleString('default', { month: 'long' })} {currentYear} · {centres.length} centres · {activeTenants.length} active tenants
        </p>
      </div>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/dashboard/maintenance" className="text-sm" style={{ color: 'var(--color-brand-light)', textDecoration: 'none' }}>
            View all →
          </Link>
        </div>
              ...
              return (
                <Link key={job.id} href={`/dashboard/maintenance/${job.id}`}
                  ...
                  <p className="text-xs mt-1" style={{ color: '#aaa' }}>
                    {(job.centre as any)?.name} · {new Date(job.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                  </p>
                </Link>
              )
            })}
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: '#fef3dc', color: '#7d5a00' }}>
              Camera API not connected
            </span>
          </div>
        ...
        <div className="rounded-xl p-5" style={{ background: '#fff', border: '1px solid #ece9e3' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-2xl font-semibold" style={{ color: '#1a1a18' }}>
                {footTrafficData.reduce((a, b) => a + b, 0).toLocaleString()}
              </p>
            ...
            <div className="text-right">
              <p className="text-sm font-medium" style={{ color: '#1a1a18' }}>
                ~{Math.round(footTrafficData.reduce((a, b) => a + b, 0) / 30).toLocaleString()} /day avg
              </p>
              <p className="text-xs" style={{ color: '#888' }}>daily average</p>
            </div>
          </div>
          <LineChart data={footTrafficData} color="#1a472a" />
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full font-medium"
              ...
              {expiringTenants.length} expiring in 60 days
            </span>
          </div>
          <Link href="/dashboard/tenants" className="text-sm" style={{ color: 'var(--color-brand-light)', textDecoration: 'none' }}>
            View all tenants →
          </Link>
        </div>
        ...
        {expiringTenants.length === 0 ? (
          <div className="rounded-xl p-6 text-center" style={{ background: '#fff', border: '1px solid #ece9e3' }}>
            ...
            <p className="text-xs mt-1" style={{ color: '#aaa' }}>
              Add lease end dates to tenants to track renewals.
            </p>
          </div>
        ) : (
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #ece9e3', background: '#fff' }}>
            {expiringTenants.map((tenant, i) => {
              const daysLeft = Math.ceil((new Date(tenant.lease_end!).getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
              return (
                <Link key={tenant.id} href={`/dashboard/tenants/${tenant.id}`}
                  ...
                  <span className="text-xs font-medium px-2.5 py-1 rounded-full"
                    style={{
                      background: daysLeft <= 14 ? '#fff0f0' : daysLeft <= 30 ? '#fef3dc' : '#f5f5f5',
                      color: daysLeft <= 14 ? '#c0392b' : daysLeft <= 30 ? '#7d5a00' : '#888',
                    }}>
                    {daysLeft} days
                  </span>
                </Link>
      ...
      <div className="grid grid-cols-2 gap-5 mb-8">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/dashboard/internet" className="text-sm" style={{ color: 'var(--color-brand-light)', textDecoration: 'none' }}>
              Manage →
            </Link>
          </div>
          <div className="rounded-xl p-5" style={{ background: '#fff', border: '1px solid #ece9e3' }}>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                ...
                { label: 'Profit', value: `$${totalProfit.toFixed(0)}` },
              ].map(s => (
            ...
            {internetStats.length === 0 ? (
              ...
              internetStats.map((s, i) => (
                <div key={s.centre_name} className="flex items-center justify-between py-2"
                  style={{ borderTop: i > 0 ? '1px solid #f0ede7' : 'none' }}>
                  <div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/dashboard/generator" className="text-sm" style={{ color: 'var(--color-brand-light)', textDecoration: 'none' }}>
              Manage →
            </Link>
          </div>
          <div className="rounded-xl p-5" style={{ background: '#fff', border: '1px solid #ece9e3' }}>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[
                { label: 'Total kWh', value: totalKwh.toFixed(0) },
                { label: 'Total cost', value: `$${generatorStats.reduce((s, g) => s + g.total_cost, 0).toFixed(0)}` },
              ].map(s => (
                <div key={s.label} className="rounded-lg p-3" style={{ background: '#f7f6f3' }}>
                  <p className="text-xs" style={{ color: '#888' }}>{s.label}</p>
                  <p className="text-base font-semibold mt-0.5" style={{ color: '#1a1a18' }}>{s.value}</p>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/dashboard/centres" className="text-sm" style={{ color: 'var(--color-brand-light)', textDecoration: 'none' }}>
            View all →
          </Link>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {centres.map(centre => {
            const count = tenants.filter((t: any) => t.centre_id === centre.id || (t.centre as any)?.name === centre.name).length
            return (
