...
const navSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/dashboard/rent', label: 'Rent', icon: Home },
      { href: '/dashboard/ops', label: 'Ops charges', icon: Wrench },
    ]
  },
  {
    label: 'UTILITIES',
...
export default function Sidebar({ profile }: SidebarProps) {
  ...
  return (
    <aside
      ...
      style={{ width: '256px', background: 'var(--color-brand)' }}
    >
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-4">
        {navSections.map((section) => (
          <div key={section.label}>
            <p className="text-xs font-medium px-2 mb-1.5"
      ...
      <div className="px-3 py-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        ...
        {/* Profile */}
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg mt-1"
          ...
          <button onClick={handleLogout} className="p-1 rounded transition-colors"
            style={{ color: 'rgba(255,255,255,0.35)' }} title="Sign out"
